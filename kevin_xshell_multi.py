...
def my_main():
  ...
  xlog_filename = "SHELL_MULTI_" + str(TODAY) + "xlsx"
  xlog_wsname = "LOG"

  if os.path.isfile(xlog_filename):
    XLOG_WB = openpyxl.load_workbook(xlog_filename)
    XLOG = XLOG_WB[xlog_wsname]
    print("found XLOG workbook", xlog_filename)
  else:
